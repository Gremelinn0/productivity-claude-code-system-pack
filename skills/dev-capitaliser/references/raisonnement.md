...
> Annexe de **`/dev-capitaliser`**. À ouvrir quand on écrit ou qu'on relit le bloc
> `<!-- raisonnement -->` d'un morceau, et pour dérouler le **mode PASSE**.
>
> **Origine** : cette annexe est l'ancienne compétence `/dev-capitaliser`, fusionnée dans
> `/dev-capitaliser`. Le geste de garder le POURQUOI et celui de garder le FAIT sont le même geste :
...
| `/dev-capitaliser` | trouve ce qui devient faux et route vers les propriétaires | **le routage** — ici on écrit chez le propriétaire une fois qu'il est connu |
...
arrive ici. `/dev-capitaliser` (global) reste le réflexe amont : elle cherche ce qui devient faux et
